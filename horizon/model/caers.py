...
Copied from https://github.com/ndkhanh360/CAER/blob/master/CAER/model/model.py
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.convs = nn.ModuleList([
            nn.Conv2d(num_kernels[i],
                      num_kernels[i + 1],
                      kernel_size,
                      padding=padding) for i in range(n)
        ])
        self.bn = nn.ModuleList(
            [nn.BatchNorm2d(num_kernels[i + 1])
...
class TwoStreamNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.attention_inference_module = Encoder([256, 128, 1],
                                                  max_pool=False)
...
class FusionNetwork(nn.Module):
    ...
    def forward(self, face, context):
        ...
        context = F.avg_pool2d(context,
                               context.shape[2]).view(context.shape[0], -1)
...
class CAERNetS(BaseModel):
    ...
    def forward(self, face=None, context=None):
        face, context, attn = self.two_stream_net(face, context)

        return self.fusion_net(face, context), attn
